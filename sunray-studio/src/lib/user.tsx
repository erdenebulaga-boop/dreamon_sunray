...
type UserContextType = {
  ...
  deleteAccount: () => void;
  createGuestUser: (info: { firstName: string; lastName: string; email: string; phone: string }) => void;
  confirmAccount: (password: string) => void;
...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const register = useCallback((newUser: User, _password: string) => {
    setUser({ ...newUser, status: "confirmed" });
    setOrders([]);
  }, []);

  const createGuestUser = useCallback((info: { firstName: string; lastName: string; email: string; phone: string }) => {
    setUser({
  ...
  const confirmAccount = useCallback((_password: string) => {
    setUser((prev) => (prev ? { ...prev, status: "confirmed" } : null));
  }, []);
